...
(function($) {
	...
	jQuery.fn.addTag = function(value,options) {
			...
			this.each(function() {
				...
				if (value !='') {
					...
					element = $('<span class="tag">'+value + '&nbsp;&nbsp;<a href="#" title="Remove tag" onclick="return $(\'#'+id + '\').removeTag(\'' + escape(value) + '\');">x</a></span>').hide().fadeIn('slow').insertBefore('#'+id+'_addTag');
	...
	jQuery.fn.tagsInput = function(options) {
		...
		var settings = jQuery.extend({defaultText:'add a tag',width:'auto',height:'auto','hide':true,'delimiter':',',autocomplete:{selectFirst:false},prefix:''},options);
		...
		this.each(function() {
			...
			$(data.fake_input).bind('keypress',data,function(event) {
				if (event.which==event.data.delimiter.charCodeAt(0) || event.which==13) {
					...
					$(event.data.real_input).addTag(settings.prefix+$(event.data.fake_input).val(),{focus:true});
					return false;
				}
			});
